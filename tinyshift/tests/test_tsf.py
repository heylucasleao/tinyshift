...
def test_tsf_evaluator_aggregates_distribution_scores(gamma_distribution):
    frame = pd.DataFrame(
        {
            "unique_id": ["A", "A", "B", "B"],
            "ds": pd.to_datetime(
            ...
            "lambda_t": [2.0, 3.0, 4.0, 5.0],
        }
    )
    distribution = GammaPredictiveDistribution(
        means=[2.0, 3.0, 4.0, 5.0],
        dispersions=[3.0, 3.0, 5.0, 5.0],
    )
    forecast = PanelPredictiveForecast(
        frame,
        distribution,
    ...
    rows = TwoStageForecasterEvaluator.evaluate_distribution(
        forecast, evaluation, train, agg=None
    )
    by_horizon = TwoStageForecasterEvaluator.evaluate_distribution(
        forecast, evaluation, train, agg="horizon"
    )
    overall = TwoStageForecasterEvaluator.evaluate_distribution(
        forecast, evaluation, train, agg="overall"
    )
    ...
    assert rows["horizon"].tolist() == [1, 2, 1, 2]
    assert by_horizon["horizon"].tolist() == [1, 2]
    assert by_horizon["n_obs"].tolist() == [2, 2]
    assert by_horizon.loc[0, "crps"] == pytest.approx(
        rows.loc[rows["horizon"] == 1, "crps"].mean()
    )
    assert by_horizon.loc[0, "ncrps"] == pytest.approx(
        rows.loc[rows["horizon"] == 1, "ncrps"].mean()
    )
    assert list(overall.columns) == ["crps", "ncrps", "n_obs"]
    assert overall.loc[0, "crps"] == pytest.approx(rows["crps"].mean())
    assert overall.loc[0, "ncrps"] == pytest.approx(rows["ncrps"].mean())
    assert overall.loc[0, "n_obs"] == 4


def test_tsf_evaluator_rejects_invalid_distribution_aggregation(gamma_distribution):
    frame = pd.DataFrame(
        {"unique_id": ["A"], "ds": [1], "lambda_t": [2.0]}
    )
    forecast = PanelPredictiveForecast(
        frame, gamma_distribution, "lambda_t", "unique_id", "ds"
    )
    evaluation = frame[["unique_id", "ds"]].assign(y=[2.0])
    train = pd.DataFrame({"unique_id": ["A", "A"], "y": [1.0, 3.0]})

    with pytest.raises(ValueError, match="agg must be one of"):
        TwoStageForecasterEvaluator.evaluate_distribution(
            forecast, evaluation, train, agg="window"
        )
...
def test_distribution_aligns_scalar_row_grid_and_matrix(gamma_distribution):
    ...
    assert gamma_distribution.cdf([2.0, 4.0]).shape == (2, 2)
    assert gamma_distribution.cdf([1.0, 2.0, 3.0]).shape == (2, 3)
    assert gamma_distribution.cdf([[1.0, 2.0], [3.0, 4.0]]).shape == (2, 2)

    with pytest.raises(ValueError, match="scalar, a one-dimensional grid"):
        gamma_distribution.cdf(np.ones((3, 2)))
    with pytest.raises(ValueError, match="finite values"):
        gamma_distribution.cdf(np.nan)


def test_predict_distribution_returns_self_contained_panel_forecast(sample_train_data):
    wrapper = TwoStageForecasterWrapper(
        MLForecast(models=[LinearRegression()], freq="D", lags=[1])
    ).fit(sample_train_data)
...
def test_continuous_panel_forecast_does_not_expose_pmf(sample_continuous_data):
    wrapper = TwoStageForecasterWrapper(
        MLForecast(models=[LinearRegression()], freq="D", lags=[1]),
        distribution=GammaFamily(),
    ).fit(sample_continuous_data)
